...
|---|---|---|---|
| 2026-09-16 | **A partner is only a first and last name** – the birth year, e-mail, phone and note were removed from the Couple screen, the schema and the stored document | Owner's request to keep the app from bloating. The planner never used the fields, they made the only form on the screen three times longer, and they were personal data kept for nothing. Old documents lose them on the next save; the privacy policy no longer lists them. | [weddyWedding.md](domains/weddyWedding.md#rules), [personalData.md](architecture/personalData.md) |
